function [fitness_raw, arx, arxvalid, arz, counteval, surrogateStats] = surrogateManager(weights,mu,xmean, sigma, lambda,B, BD, diagD, countiter, fitfun_handle, inOpts, varargin)
...
% @surrogateStats       vector of numbers with variable model/surrogate statsitics
  ...
  persistent lastReduceModel;         % last successfully trained model
  ...
  elseif (strcmpi(surrogateOpts.evoControl, 'generation'))
    ...
    if(true)%(generationEC.trainModel)
        [newModel, newReduceModel, surrogateStats, isTrained] = trainGenerationECModel(weights,mu,newModel, archive, xmean, sigma, lambda, BD, diagD, surrogateOpts, countiter);
        if (isTrained)
    ...
    if (true)%(generationEC.evaluateOriginal)
      %
      % original-evaluated generation
      %
...
%       end       % ~ generationEC.isNextOriginal()
      ...
      if (~isempty(shiftedModel))
        ...
        surrogateStats = getModelStatistics(weights,mu,shiftedModel,shiftedReduceModel, xmean, sigma, lambda, BD, diagD, surrogateOpts, countiter);
...
%         [lamda2,~]=size(predictReduceOrder);
         ...
         bad=badness(correctOrder(1:mu),predictOrder(1:mu),weights,lambda);
         badReduce=badness(correctOrder(1:mu),predictReduceOrder(1:mu),weights,lambda);
...
function [newModel,newReduceModel, surrogateStats, isTrained] = trainGenerationECModel(weights,mu,model, archive, xmean, sigma, lambda, BD, diagD, surrogateOpts, countiter)
surrogateStats = NaN(1, 10);
% train the 'model' on the relevant data in 'archive'
  ...
  if (length(y) >= nRequired)
    ...
    [newModel,newReduceModel] = model.train(X, y, xmean', countiter, sigma, BD);
    ...
    if (isTrained)
      ...
      surrogateStats = getModelStatistics(weights,mu,newModel,newReduceModel, xmean, sigma, lambda, BD, diagD, surrogateOpts, countiter);
%       preciseModel = ModelFactory.createModel('bbob', surrogateOpts.modelOpts, xmean');
%       helper(preciseModel,BD,xmean',30)
    end
  else
    newModel = model;
    newReduceModel = model;
    isTrained = false;
  end
end
